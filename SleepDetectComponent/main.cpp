...
using SleepDetectComponentClientPtr = boost::shared_ptr<base::network::AbstractClient>;
...
SleepDetectComponentClientPtr gSleepDetectComponentClientPtr;
	...
	if (eSuccess == cl.parseCommandLine(argc, argv))
	{
		...
		if (configName.empty())
		{
			if (name)
			{
				...
				//名字直接写入配置文件,需要再去读
				XMLPacker().setValueWithName("Config.xml", "Component.Sleep.Name", name);
			}
		}
...
{
	int e{ gSleepDetectComponentClientPtr ? eObjectExisted : eSuccess };
	...
	if (eSuccess == e)
	{
		...
		{
			SleepDetectComponentClientPtr acp{ 
				boost::make_shared<SleepDetectComponentClient>(gMessageDispatcherCenterIP) };
			if (acp)
			{
				e = acp->startClient((boost::format("tcp://%s:%d") % gMessageDispatcherCenterIP % gMessageDispatcherCenterPort).str(), "Sleep");

				if (eSuccess == e)
				{
					gSleepDetectComponentClientPtr.swap(acp);
				}
				else
				{
					e = eBadOperate;
...
{
	int e{ gSleepDetectComponentClientPtr ? eSuccess : eBadOperate };

	if (eSuccess == e)
	{
		e = gSleepDetectComponentClientPtr->stopClient();

		if (eSuccess == e)
		{
			gSleepDetectComponentClientPtr.reset();
		}
		else
		{
			e = eBadOperate;
