...
using HelmetDetectComponentClientPtr = boost::shared_ptr<base::network::AbstractClient>;
...
HelmetDetectComponentClientPtr gHelmetDetectComponentClientPtr;
...
{
	int e{ gHelmetDetectComponentClientPtr ? eObjectExisted : eSuccess };
	...
	if (eSuccess == e)
	{
			...
			gMaxPortNumber > gMessageDispatcherCenterPort)
		{
			...
			if (acp)
			{
				...
				if (eSuccess == e)
				{
					gHelmetDetectComponentClientPtr.swap(acp);
				}
				else
				{
					e = eBadOperate;
...
{
	int e{ gHelmetDetectComponentClientPtr ? eSuccess : eBadOperate };

	if (eSuccess == e)
	{
		e = gHelmetDetectComponentClientPtr->stopClient();

		if (eSuccess == e)
		{
			gHelmetDetectComponentClientPtr.reset();
		}
		else
		{
			e = eBadOperate;
