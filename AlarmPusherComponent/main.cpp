...
using AlarmPusherComponentClientPtr = boost::shared_ptr<base::network::AbstractClient>;

static std::string gMessageDispatcherCenterIP{ "127.0.0.1" };
static unsigned short gMessageDispatcherCenterPort{ 61001 };
AlarmPusherComponentClientPtr gAlarmPusherComponentClientPtr;
...
{
	int e{ gAlarmPusherComponentClientPtr ? eObjectExisted : eSuccess };
	...
	if (eSuccess == e)
	{
		if (!gMessageDispatcherCenterIP.empty() && gMinPortNumber < gMessageDispatcherCenterPort && gMaxPortNumber > gMessageDispatcherCenterPort)
		{
			...
			if (ap)
			{
				...
				if (eSuccess == e)
				{
					gAlarmPusherComponentClientPtr.swap(ap);
				}
				else
				{
					e = eBadOperate;
...
{
	int e{ gAlarmPusherComponentClientPtr ? eSuccess : eBadOperate };

	if (eSuccess == e)
	{
		e = gAlarmPusherComponentClientPtr->stopClient();

		if (eSuccess == e)
		{
			gAlarmPusherComponentClientPtr.reset();
		}
		else
		{
			e = eBadOperate;
