	...
	const unsigned short port /* = 0 */)
{
	...
	if (eSuccess == e)
	{
		LOG(INFO) << "Start sleep with parameters of command line [ name = " << getName() <<
			", remote XMQ IPv4 = " << ipv4 <<
			", remote XMQ port = " << port << " ].";
	}
	else
	{
		LOG(WARNING) << "Start sleep with parameters of command line [ name = " << getName() <<
			", remote XMQ IPv4 = " << ipv4 <<
			", remote XMQ port = " << port << " ].";
	}
...
int Sleep::stopOD()
{
	...
	if (eSuccess == e)
	{
		LOG(INFO) << "Stop sleep service successfully.";
	}
	else
	{
		LOG(WARNING) << "Stop sleep service failed, result = " << e << ".";
	}

	return e;
}

void Sleep::afterParsePolledMessage(
	...
	const std::vector<std::string> messages)
{
	...
	{
		LOG(WARNING) << "Sleep polled unknown command type of message with from [ " << from <<
			" ] to [ " << to <<
			" ] command [ " << command << " ].";
	}
}

const std::string Sleep::buildRegisterMessage()
{
...
void Sleep::processComponentMessage(
	const std::string module,
	const std::string from,
	const std::string to,
	...
	const std::string msg)
{
	...
	if (eSuccess == e)
	{
		if (2 == cm.command)
		{
			...
			LOG(INFO) << "Sleep process component message with module [  " << module <<
				" ] from [ " << from <<
				" ] to [ " << to <<
				" ], sequence = " << sequence <<
				", timestamp = " << timestamp <<
	...
	{
		LOG(WARNING) << "Sleep process component message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
...
void Sleep::processAlgorithmMessage(
	const std::string module,
	const std::string from,
	const std::string to,
	const std::vector<std::string> routers,
	...
	const std::string msg)
{
	...
	if (eSuccess == e)
	{
		if (1 == am.command)
		{
			...
			if (graph && eSuccess == graph->runGraph())
			{
				...
				cvp.sleepTime = am.algos[0].triggertimeforsleep;
				...
				if (fp)
				{
					fp->setDataNotificationCallback(boost::bind(&Sleep::afterEncodeJPEGImageCallback, this, _1));
				}
		...
		LOG(INFO) << "Sleep process algorithm message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
	...
	{
		LOG(WARNING) << "Sleep process algorithm message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
	...
	{
		LOG(INFO) << "Sleep send message of device successfully.";
	}
	else
	{
		LOG(WARNING) << "Sleep send message of device failed.";
	}
}

void Sleep::processStatusMessage(
	const std::string module,
	const std::string from,
	const std::string to,
	const std::vector<std::string> routers,
	...
	const std::string msg)
{
	...
	if (eSuccess == e)
	{
		...
		LOG(INFO) << "Sleep process status message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
	...
	{
		LOG(WARNING) << "Sleep process status message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
...
void Sleep::afterEncodeJPEGImageCallback(DataPtr data)
{
	if (workState)
	{
		FramePtr fp{ 
		...
		almmsg.alarm.type = ot.enable ? 6 : 3;
		almmsg.alarm.bodycount = ot.otps.size();
		if (0 < ot.otps.size())
		{
		...
		msg_.addMessage(gSleepComponentName);
