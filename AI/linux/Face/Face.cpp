	...
	const unsigned short port /* = 0 */)
{
	...
	if (eSuccess == e)
	{
		LOG(INFO) << "Start face with parameters of command line [ name = " << getName() <<
			", remote XMQ IPv4 = " << ipv4 <<
			", remote XMQ port = " << port << " ].";
	}
	else
	{
		LOG(WARNING) << "Start face with parameters of command line [ name = " << getName() <<
			", remote XMQ IPv4 = " << ipv4 <<
			", remote XMQ port = " << port << " ].";
	}
...
int Face::stopOD()
{
	...
	if (eSuccess == e)
	{
		LOG(INFO) << "Stop face service successfully.";
	}
	else
	{
		LOG(WARNING) << "Stop face service failed, result = " << e << ".";
	}

	return e;
}

void Face::afterParsePolledMessage(
	...
	const std::vector<std::string> messages)
{
	...
	{
		processCrewMessage(module, from, to, routers, sequence, timestamp, msg);
	}
	else
	{
		LOG(WARNING) << "Face polled unknown command type of message with from [ " << from <<
			" ] to [ " << to <<
			" ] command [ " << command << " ].";
	}
}

const std::string Face::buildRegisterMessage()
{
...
void Face::processComponentMessage(
	const std::string module,
	const std::string from,
	const std::string to,
	...
	const std::string msg)
{
	...
	if (eSuccess == e)
	{
		if (2 == cm.command)
		{
			...
			LOG(INFO) << "Face process component message with module [  " << module <<
				" ] from [ " << from <<
				" ] to [ " << to <<
				" ], sequence = " << sequence <<
				", timestamp = " << timestamp <<
	...
	{
		LOG(WARNING) << "Face process component message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
...
void Face::processAlgorithmMessage(
	const std::string module,
	const std::string from,
	const std::string to,
	const std::vector<std::string> routers,
	...
	const std::string msg)
{
	...
	if (eSuccess == e)
	{
		if (1 == am.command)
		{
			...
			if (graph && eSuccess == graph->runGraph())
			{
				...
				cvp.matchThreshold = am.algos[0].similarforface;
				...
				if (fp)
				{
					fp->setDataNotificationCallback(boost::bind(&Face::afterEncodeJPEGImageCallback, this, _1));
				}
		...
		LOG(INFO) << "Face process algorithm message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
	...
	{
		LOG(WARNING) << "Face process algorithm message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
	...
	{
		LOG(INFO) << "Face send message of device successfully.";
	}
	else
	{
		LOG(WARNING) << "Face send message of device failed.";
	}
}

void Face::processStatusMessage(
	const std::string module,
	const std::string from,
	const std::string to,
	const std::vector<std::string> routers,
	...
	const std::string msg)
{
	...
	if (eSuccess == e)
	{
		...
		LOG(INFO) << "Helmet process status message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
	...
	{
		LOG(WARNING) << "Helmet process status message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
	...
	const std::string msg)
{
	...
	CrewMsg cm;
	int e{ ProtocolCrew().unpack(msg, cm) };
	...
	if (eSuccess == e)
	{
		if (1 == cm.command)
		{
			...
			algorithms.values(graphs);
			for (int i = 0; i != graphs.size(); ++i)
			{
				FilterPtr fp{ 
					graphs[i]->queryFilter(framework::multimedia::CVCompuptingFilterName) };
				if (fp)
				{
					boost::shared_ptr<CVComputingFilter> cvfp{
						boost::dynamic_pointer_cast<CVComputingFilter>(fp) };
		...
		LOG(INFO) << "Face process crew message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp << ".";
	...
	{
		LOG(WARNING) << "Face process crew message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
	...
	//-------------------------------------------------------------------------------------------
	Msg msg_;
	msg_.addMessage(ProtocolCrew().pack(cm));
	msg_.addMessage(gStatusCommandName);
	msg_.addMessage(timestamp);
	msg_.addMessage(sequence);
	msg_.addMessage(gEmptyData);
	...
	if (eSuccess == OD::sendMsg(&msg_))
	{
		LOG(INFO) << "Face send message of crew successfully.";
	}
	else
	{
		LOG(WARNING) << "Face send message of crew failed.";
	}
}

void Face::afterEncodeJPEGImageCallback(DataPtr data)
{
	if (workState)
	{
		FramePtr fp{
		...
		almmsg.alarm.idforface = fp->faceID;

		//---------------------------------------------------------------------------------------------
		//| "" | "worker" | "sleep" | "cms" | sender | "" | sequence | timestamp | "alarm" | message |
		//---------------------------------------------------------------------------------------------
		Msg msg_;
		...
		msg_.addMessage(gFaceComponentName);
		msg_.addMessage(gWorkerModuleName);
		msg_.addMessage(gEmptyData);

		if (eSuccess == OD::sendMsg(&msg_))
		{
			LOG(INFO) << "Sleep send alarm message successfully.";
		}
		else
		{
			LOG(WARNING) << "Sleep send alarm message failed.";
		}
	}
}
