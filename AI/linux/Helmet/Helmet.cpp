	...
	const unsigned short port /* = 0 */)
{
	...
	if (eSuccess == e)
	{
		LOG(INFO) << "Start helmet with parameters of command line [ name = " << getName() <<
			", remote XMQ IPv4 = " << ipv4 <<
			", remote XMQ port = " << port << " ].";
	}
	else
	{
		LOG(WARNING) << "Start helmet with parameters of command line [ name = " << getName() <<
			", remote XMQ IPv4 = " << ipv4 <<
			", remote XMQ port = " << port << " ].";
	}
...
int Helmet::stopOD()
{
	...
	if (eSuccess == e)
	{
		LOG(INFO) << "Stop helmet service successfully.";
	}
	else
	{
		LOG(WARNING) << "Stop helmet service failed, result = " << e << ".";
	}

	return e;
}

void Helmet::afterParsePolledMessage(
	...
	const std::vector<std::string> messages)
{
	...
	{
		LOG(WARNING) << "Helmet polled unknown command type of message with from [ " << from <<
			" ] to [ " << to <<
			" ] command [ " << command << " ].";
	}
}

const std::string Helmet::buildRegisterMessage()
{
...
void Helmet::processComponentMessage(
	const std::string module,
	const std::string from,
	const std::string to,
	...
	const std::string msg)
{
	...
	if (eSuccess == e)
	{
		if (2 == cm.command)
		{
			...
			LOG(INFO) << "Helmet process component message with module [  " << module <<
				" ] from [ " << from <<
				" ] to [ " << to <<
				" ], sequence = " << sequence <<
				", timestamp = " << timestamp <<
	...
	{
		LOG(WARNING) << "Helmet process component message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
...
void Helmet::processAlgorithmMessage(
	const std::string module,
	const std::string from,
	const std::string to,
	const std::vector<std::string> routers,
	...
	const std::string msg)
{
	...
	if (eSuccess == e)
	{
		if (1 == am.command)
		{
			...
			if (graph && eSuccess == graph->runGraph())
			{
				...
				if (fp)
				{
					fp->setDataNotificationCallback(boost::bind(&Helmet::afterEncodeJPEGImageCallback, this, _1));
				}
		...
		LOG(INFO) << "Helmet process algorithm message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
	...
	{
		LOG(WARNING) << "Helmet process algorithm message with module [  " << module <<
			" ] from [ " << from <<
			" ] to [ " << to <<
			" ], sequence = " << sequence <<
			", timestamp = " << timestamp <<
	...
	{
		LOG(INFO) << "Helmet send message of device successfully.";
	}
	else
	{
		LOG(WARNING) << "Helmet send message of device failed.";
	}
}

void Helmet::processStatusMessage(
	const std::string module,
	const std::string from,
	const std::string to,
	const std::vector<std::string> routers,
...
void Helmet::afterEncodeJPEGImageCallback(DataPtr data)
{
	...
	msg_.addMessage(gHelmetComponentName);
	msg_.addMessage(gWorkerModuleName);
	msg_.addMessage(gEmptyData);

	if (eSuccess == OD::sendMsg(&msg_))
	{
		LOG(INFO) << "Helmet send alarm message successfully.";
	}
	else
	{
		LOG(WARNING) << "Helmet send alarm message failed.";
	}
}
