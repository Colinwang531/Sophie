...
{
	gApplicationID.append(XmlParser().getValue("config.xml", "Component.Clock.ID"));

	if (gApplicationID.empty())
	{
		gApplicationID.append(Uuid().generateRandomUuid());
		XmlParser().setValue("config.xml", "Component.Clock.ID", gApplicationID);
	}
...
int main(int argc, char* argv[])
{
	...
	{
		Clock clock{log, gApplicationID, &ctx};
		clock.start(gRemoteIP, gRemotePort, gComNumber, gComBaudrate);
		getchar();
		clock.stop();
	}
